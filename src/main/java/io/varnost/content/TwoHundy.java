...
public class TwoHundy implements RuleInterface {
    @Override
    public DataStream<Alert> logic(DataStream<ObjectNode> stream) {
        DataStream<ObjectNode> filtered = Content.filter(stream, "response", "200");

        DataStream<List<ObjectNode>> aggregated = Content.aggregate(filtered, Time.minutes(1), Time.seconds(1), 10);
